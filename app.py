...
# app.py — NeuroShield demo UI matching your mock, with progressive tabs + doc scanner

import streamlit as st
from time import perf_counter
from firewall_graph import build_firewall_graph
from patterns import KEYWORD_PATTERNS, REGEX_PATTERNS, SECRET_PATTERNS
import fitz, docx, re, json, os, math
from collections import defaultdict

st.set_page_config(page_title="NeuroShield Security Suite", layout="wide", page_icon="🛡️")

# ───────── Utilities for doc scanner (your original functions) ─────────
def extract_text_from_file(uploaded_file):
    if uploaded_file.name.endswith('.pdf'):
        try:
            doc = fitz.open(stream=uploaded_file.getvalue(), filetype="pdf")
...
def analyze_text(text):
    ...
    for category, keywords in KEYWORD_PATTERNS.items():
        for keyword in keywords:
            ...
            if count > 0: results[f"{category}: {keyword}"] = count
    for pn, pr in REGEX_PATTERNS.items():
        m = re.findall(pr, text, re.IGNORECASE)
        if m: results[pn] = len(m)
    for pn, pr in SECRET_PATTERNS.items():
        flags = re.IGNORECASE
        if "Private Key" in pn: flags |= re.DOTALL
        if "Generic Secret" in pn: flags |= re.VERBOSE
        try:
            m = re.findall(pr, text, flags)
            if m: results[f"SECRET: {pn}"] = len(m)
        except re.error as e:
            st.error(f"Error compiling regex for '{pn}': {e}")
    potential_secrets = re.split(r'[\s\'".,;=()\[\]{}]', text)
    high_entropy_strings = 0
    for s in potential_secrets:
        if 20 <= len(s) <= 64 and calculate_shannon_entropy(s) > 4.5:
            high_entropy_strings += 1
    if high_entropy_strings > 0: results["SECRET: High-Entropy String"] = high_entropy_strings
    return results

def generate_detection_report(detected_secrets, all_secret_patterns):
    report_data = {}
    for pattern_name in detected_secrets.keys():
        original_pattern_name = pattern_name.replace("SECRET: ", "")
        if original_pattern_name in all_secret_patterns:
            report_data[pattern_name] = all_secret_patterns[original_pattern_name]
    return json.dumps(report_data, indent=4)

# ───────── Header ─────────
st.markdown(
    "<h1 style='display:flex;align-items:center;gap:.5rem'>🛡️ NeuroShield Security Suite</h1>",
    unsafe_allow_html=True
)

# Two top tabs
top_tab = st.tabs(["🔐 Prompt Firewall", "📄 Document Scanner"])
...
with top_tab[0]:
    ...
    with colL:
        ...
        user_prompt = st.text_area(
            ...
            height=160, placeholder="Type a prompt to analyze…"
        )

        paste_toggle = st.toggle("Paste existing LLM response")
        pasted_resp = st.text_area(" ", height=140, label_visibility="collapsed") if paste_toggle else None

        analyze = st.button("Analyze Security", type="primary", disabled=not user_prompt.strip())

    with colR:
        st.subheader("Performance")
        c1, c2 = st.columns(2)
        with c1:
            st.metric("ContextAnalyzer", "92%", "Improve context handling")
            st.metric("RiskScorer", "95%", "Refine thresholds")
        with c2:
            st.metric("RegexFilter", "87%", "Review patterns")
            st.metric("FinalVerdict", "91%", "Decision rules")

    # Flow tabs (left-to-right, match your mock)
    flow_tabs = st.tabs(["ContextAnalyzer", "RegexFilter", "RiskScorer", "FinalVerdict"])
    tab_map = {
        "ContextAnalyzer": flow_tabs[0],
        "RegexFilter": flow_tabs[1],
        "RiskScorer": flow_tabs[2],
        "FinalVerdict": flow_tabs[3],
    }
    ...
    if analyze:
        ...
        start = perf_counter()
        final_state = {}
        for node, state in runner({
            "user_prompt": user_prompt,
            "pasted_llm_response": pasted_resp.strip() if pasted_resp else None
        }):
            final_state = state
            # Write JSON to the matching tab as the step completes
            if node in tab_map:
                with tab_map[node]:
                    st.json(state)
...
with top_tab[1]:
    st.subheader("Upload a document to scan for sensitive content")
    uploaded_file = st.file_uploader("Choose a file", type=["pdf", "docx", "txt"])
    if uploaded_file is not None:
        with st.spinner("Analyzing…"):
            document_text = extract_text_from_file(uploaded_file)
            if document_text:
                pattern_results = analyze_text(document_text)
            else:
                pattern_results = {}
        if pattern_results:
            st.write("### Full Scan Results")
            st.table({"Pattern / Indicator": list(pattern_results.keys()),
                      "Occurrences": list(pattern_results.values())})
            sensitive = {k: v for k, v in pattern_results.items() if k.startswith("SECRET:")}
            st.divider()
            if sensitive:
                st.error("🚨 Potential secrets found")
                for pn in sensitive.keys(): st.markdown(f"- **{pn}**")
                json_report_str = generate_detection_report(sensitive, SECRET_PATTERNS)
                os.makedirs("warning_reports", exist_ok=True)
                path = os.path.join("warning_reports", f"warning_report_{uploaded_file.name}.json")
                with open(path, "w") as f: f.write(json_report_str)
                st.download_button("Download Warning Report (JSON)", json_report_str,
                                   file_name=f"warning_report_{uploaded_file.name}.json",
                                   mime="application/json")
            else:
                st.success("✅ No secrets detected")
                os.makedirs("reviewed_docs", exist_ok=True)
                safe_path = os.path.join("reviewed_docs", uploaded_file.name)
                with open(safe_path, "wb") as f:
                    f.write(uploaded_file.getbuffer())
                st.info(f"Saved to `{safe_path}`")

# Bottom demo buttons (no-op stubs for now)
st.write("---")
b1, b2 = st.columns(2)
b1.button("Generate Flowchart")
b2.button("Generate Summary")
